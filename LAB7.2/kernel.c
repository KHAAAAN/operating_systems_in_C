...
PROC *kfork(char *filename){
	...
	for(i = 0; i < NFD; i++){ //same modifications to vfork();
		if(running->fd[i]){ // copy only non-zero entires
			p->fd[i] = running->fd[i];
...
int tsleep(int t){
    if(running->pid <= 1){
      printf("ERROR: Cannot put PROC1 to sleep.\n");
      return -1;
    }

    running->time = t;
    printf("tsleep for %d seconds\n", running->time);
    ksleep(running + 1); //running + 1 because we have running elsewhere
}
